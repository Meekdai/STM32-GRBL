...
char EE_ReadBuffer(void *data, uint16_t length, uint16_t addr)
{
		char buffer[100];
		uint16_t d,pos = 0;
		do
		{
				if(EE_ReadVariable(addr + pos,&d)) return 0;
				memcpy(&buffer[pos * 2],&d,2);	
				pos++;
		}while(length -= 2);	
		memcpy(data,&buffer,pos * 2);
		return 1;
}
char EE_WriteBuffer(void *data, uint16_t length, uint16_t addr)
{
		char buffer[100];
		uint16_t d,pos = 0;
		...
		do
		{
				memcpy(&d,&buffer[2 * pos],2);	
				EE_WriteVariable(addr + pos,d);
				pos++;	
		}while(length -= 2);	
		return 1;
}
